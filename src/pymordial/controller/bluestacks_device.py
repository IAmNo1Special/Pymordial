...
from pymordial.core.pymordial_emulator import EmulatorState, PymordialEmulatorDevice
...
class PymordialBluestacksDevice(PymordialEmulatorDevice):
    ...
    def __init__(
        ...
        Args:
            ...
            image_controller: The image controller used for screen analysis.
        ...
        self.state.register_handler(
            EmulatorState.READY, self._adb_bridge_device.connect, None
        )
...
if __name__ == "__main__":
    from pymordial.controller import ImageController, PymordialAdbDevice

    adb_bridge_device = PymordialAdbDevice(host="127.0.0.1", port=5555)
    image_controller = ImageController(adb_bridge_device=adb_bridge_device)
    controller = PymordialBluestacksDevice(
        adb_bridge_device=adb_bridge_device, image_controller=image_controller
    )
    controller.open()
